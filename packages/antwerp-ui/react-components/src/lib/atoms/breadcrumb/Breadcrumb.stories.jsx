...
export default {
  ...
  argTypes: {
    items: {
      control: { type: 'object' },
      defaultValue: [{label: 'Home', href: '/'}, {label: 'Google', href: 'https://www.google.be'}, {label: 'Current Page'}],
      table: {
        type: { summary: 'array' },
    ...
    renderLinkFunction: {
      ...
      description:
        'Optional (custom) link renderer; allows to override the way links are rendered. This is useful when using a routing framework (like React Router). The first parameter of this function is a `Link` object, with fields `label`, `href`. Example: `linkRenderFunction={(link) => <Link to={link.href}>{link.label}</Link>}`'
    },
    qa: QA_PROP_STORY
  },
};
